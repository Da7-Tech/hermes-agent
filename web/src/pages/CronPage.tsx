...
export default function CronPage() {
  ...
  const handlePauseResume = async (job: CronJob) => {
    try {
      ...
      if (isPaused) {
        ...
        showToast(
          `${t.cron.resume}: "${truncateText(getJobTitle(job, t.cron.jobFallback), 30)}"`,
          "success",
        );
      } else {
        await api.pauseCronJob(job.id, profile);
        showToast(
          `${t.cron.pause}: "${truncateText(getJobTitle(job, t.cron.jobFallback), 30)}"`,
          "success",
        );
      }
      loadJobs();
  ...
  const handleTrigger = async (job: CronJob) => {
    try {
      ...
      showToast(
        `${t.cron.triggerNow}: "${truncateText(getJobTitle(job, t.cron.jobFallback), 30)}"`,
        "success",
      );
      loadJobs();
    } catch (e) {
    ...
    onDelete: useCallback(
      async (key: string) => {
        ...
        try {
          ...
          showToast(
            `${t.common.delete}: "${job ? truncateText(getJobTitle(job, t.cron.jobFallback), 30) : id}"`,
            "success",
          );
          loadJobs();
        } catch (e) {
      ...
      [jobs, loadJobs, showToast, t.common.delete, t.cron.jobFallback, t.status.error],
    ),
  });
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <DeleteConfirmDialog
        ...
        description={
          pendingJob
            ? `"${truncateText(getJobTitle(pendingJob, t.cron.jobFallback), 40)}" — ${
      ...
      {view === "jobs" && (
        <div className="flex flex-col gap-3">
          ...
          {jobs.map((job) => {
            ...
            const title = getJobTitle(job, t.cron.jobFallback);
                    ...
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {Array.isArray(job.skills) && job.skills.length > 0 && (
                        <Badge tone="outline" title={job.skills.join(", ")}>
                          {job.skills.length === 1
                            ? job.skills[0]
                            : (t.cron.skillsCount ?? "{count} skills").replace(
                                "{count}",
                                String(job.skills.length),
                              )}
                        </Badge>
                      )}
                    </div>
